...
//delete one stock
router.delete("/stock/:pid_Id", async (req, res) => {
    const { pid_Id } = req.params;

    if (!pid_Id) {
        return res.status(400).json({ success: false, message: "Missing pid_Id" });
    }
    ...
    try {
        ...
        // Step 1: Get stock entry to delete
        const [stockRows] = await conn.query(
            `SELECT pc_Id, I_Id FROM p_i_detail WHERE pid_Id = ?`,
            [pid_Id]
        );
        ...
        // Step 2: Get the unit cost from purchase_detail
        const [purchaseDetailRows] = await conn.query(
            `SELECT unitPrice FROM purchase_detail WHERE pc_Id = ? AND I_Id = ?`,
            [pc_Id, I_Id]
        );
        ...
        const unitCost = parseFloat(purchaseDetailRows[0].unitPrice);

        // Step 3: Delete from p_i_detail
        await conn.query(`DELETE FROM p_i_detail WHERE pid_Id = ?`, [pid_Id]);

        // Step 4: Update purchase_detail (reduce rec_count and total by cost)
        await conn.query(
            `UPDATE purchase_detail 
       SET rec_count = GREATEST(rec_count - 1, 0),
           total = GREATEST(total - ?, 0)
       WHERE pc_Id = ? AND I_Id = ?`,
            [unitCost, pc_Id, I_Id]
        );

        // Step 5: Update Item table (stockQty, availableQty)
        await conn.query(
            `UPDATE Item 
       SET stockQty = GREATEST(stockQty - 1, 0),
           availableQty = GREATEST(availableQty - 1, 0)
       WHERE I_Id = ?`,
            [I_Id]
        );

        // Step 6: Update purchase table (total and balance reduced by unit cost)
        await conn.query(
            `UPDATE purchase 
       SET total = GREATEST(total - ?, 0),
           balance = GREATEST(balance - ?, 0)
       WHERE pc_Id = ?`,
            [unitCost, unitCost, pc_Id]
        );
        ...
        return res.status(200).json({
            success: true,
            message: "Stock item deleted and cost-adjusted purchase records updated.",
        });
